...
export const getProviders = async () => {
  ...
  try {
    ...
    console.log(
      "Supabase no configurado o sin datos, usando fallback estático para proveedores.",
    );
  }
...
export const getRequestsForProvider = async (providerId) => {
  try {
    ...
    return requests
      .filter((req) => req.providerId == providerId)
      .map((req) => ({
        id: req.id,
...
export const updateRequestStatus = async (requestId, status) => {
  ...
  try {
    ...
    console.log(
      "Fallback: actualizando solicitud en localStorage falló en Supabase",
      e.message,
    );
  }
};
...
export const getUserProfile = async (userId) => {
  try {
      ...
      if (error.code === "PGRST116") {
        ...
        if (authData && authData.user && authData.user.id === userId) {
          ...
          const newProfile = {
            ...
            description:
              role === "customer" ? "" : "Sin descripción disponible.",
          ...
          if (!insertError && insertedData) {
            ...
            console.warn(
              "Error al auto-crear perfil (quizás falta esquema), usando perfil local en memoria:",
              insertError,
            );
            return newProfile;
          }
    ...
    return {
      ...
      email:
        role === "customer" ? "cliente@ejemplo.cl" : "prestador@ejemplo.cl",
      role: role,
      avatar_url:
        "https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&w=150&q=80",
      profession: role === "customer" ? "" : "Nuevo Prestador",
...
export const updateProviderAvailability = async (
  ...
  try {
    ...
    try {
      ...
      console.warn(
        "Error consultando la tabla profiles en updateProviderAvailability:",
        profileErr,
      );
    }

    const { data, error } = await supabase.from("providers").upsert(updateData);
    if (error) throw error;
    return data;
  } catch (e) {
    ...
    if (locs.length === 0) {
      const defaults = [
        ...
        },
        {
          id: 2,
          user_id: userId,
          label: "Casa de Mamá (Providencia)",
...
export const relocateProviders = (lat, lng, forceRelocateReal = false) => {
  // Función vacía: Ya no relocalizamos artificialmente a los proveedores.
  // Los prestadores ahora aparecerán en su ubicación real.
};
...
export const getSavedProviders = (userId) => {
  ...
  try {
    const saved = JSON.parse(
      localStorage.getItem(`prolink_favorites_${userId}`) || "[]",
    );
    return saved;
  } catch (e) {
    return [];
...
export const getUserProfessions = (userId) => {
  try {
    const saved = JSON.parse(
      localStorage.getItem(`prolink_professions_${userId}`) || "[]",
    );
    return saved;
  } catch (e) {
    return [];
...
export const saveUserProfession = (userId, professionData) => {
  try {
    let saved = JSON.parse(
      localStorage.getItem(`prolink_professions_${userId}`) || "[]",
    );
    
    // Evitar duplicados si ya existe la misma subcategoría
    const exists = saved.find(p => p.subcategory === professionData.subcategory);
    if (exists) {
      return exists;
    }
    ...
    saved.push(newProfession);
    localStorage.setItem(`prolink_professions_${userId}`, JSON.stringify(saved));
    return newProfession;
  } catch (e) {
    console.error("Error saving profession", e);
    return null;
  }
};

export const setActiveProfession = async (userId, professionId) => {
  try {
    const saved = JSON.parse(
      localStorage.getItem(`prolink_professions_${userId}`) || "[]",
    );
    const profession = saved.find(p => p.id === professionId);
    ...
    // También actualizar provider availability
    await updateProviderAvailability(
      userId,
      true, // Mantener disponible
      null,
      null
    );
